...
### Added
- New `README.md` in the root directory with local testing instructions.

### Changed
- Updated Cal.com embed code in `booking.html` to the latest inline embed.

### Fixed
- Restored header styling on `booking.html` by adding the missing `css/styles.css` link.
- Improved dropdown menu text contrast in `css/styles.css` by explicitly setting hover/focus/active states.
